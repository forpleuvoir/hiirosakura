...
fun ContainerScope.AttributeModifiersComponentWrapper(
    id: Identifier,
    component: AttributeModifiersComponent,
    removeAction: () -> Unit,
    modifier: Modifier = Modifier,
    horizontalArrangement: Arrangement.Horizontal = Arrangement.spacedBy(5f, Alignment.Right),
    verticalAlignment: Alignment.Vertical = Alignment.CenterVertically,
    onValueChange: (AttributeModifiersComponent, Boolean) -> Unit,
) = DataComponentWrapperRow(id, removeAction, modifier, horizontalArrangement, verticalAlignment) {
    var component = component
    Button(
        modifier = Modifier.width(140f)
    ) {
        TextLabel(IGLang.listConfigWrapperText(component.modifiers.size))
...
fun AttributeModifiersComponentEditor(
    ...
    return DataComponentWrapperDialog(
        title,
        {
            AttributeModifiersComponent(modifiers, showInTooltip.getValue()) to true
        },
        onValueChange,
        modifier,
        screenModifier
                ...
                EntiryAttributeSelector(REGISTERED_ATTRIBUTE.first().asMutableState, modifier = Modifier.width(160f), onSelected = {
                    modifiers.addLast(
                        AttributeModifiersComponent.Entry(
                            it,
                            EntityAttributeModifier(Identifier.of("minecraft:unknow"), 0.0, EntityAttributeModifier.Operation.ADD_VALUE),
        ...
        ColumnListWrapped(
            ...
            amountStep(15f)
            if (modifiers.isEmpty()) TextLabel(IGLang.hasNothing)
...
fun ContainerScope.AttributeModifiersComponentEntryWrapper(
    entry: AttributeModifiersComponent.Entry,
    title: Text,
    onRemove: () -> Unit,
    modifier: Modifier = Modifier,
    verticalArrangement: Arrangement.Vertical = Arrangement.Center,
    horizontalAlignment: Alignment.Horizontal = Alignment.CenterHorizontally,
    onValueChange: (AttributeModifiersComponent.Entry) -> Unit
    ...
    SwitchableProxy(
        {
            Row {
                ...
                Column {
                    ...
                    type.subscribe {
                        entry = AttributeModifiersComponent.Entry(it, entry.modifier, entry.slot)
                        onValueChange(entry)
                    }
                    ...
                    id.subscribe {
                        entry =
                            AttributeModifiersComponent.Entry(
                                entry.attribute,
                                EntityAttributeModifier(it, entry.modifier.value, entry.modifier.operation),
                                entry.slot
                            )
                        onValueChange(entry)
                    }
                    val amount = entry.modifier.value.asMutableState
                    amount.subscribe {
                        entry =
                            AttributeModifiersComponent.Entry(
                                entry.attribute,
                                EntityAttributeModifier(entry.modifier.id, it, entry.modifier.operation),
                                entry.slot
                            )
                        onValueChange(entry)
                    }
                    ...
                    operation.subscribe {
                        entry =
                            AttributeModifiersComponent.Entry(entry.attribute, EntityAttributeModifier(entry.modifier.id, entry.modifier.value, it), entry.slot)
                        onValueChange(entry)
                    }

                    val slot = entry.slot.asMutableState
                    slot.subscribe {
                        entry = AttributeModifiersComponent.Entry(entry.attribute, entry.modifier, it)
                        onValueChange(entry)
                    }
                    ...
                    Row(
                        ...
                        Row(
                            ...
                            Box(
                                Modifier.width(135f).padding(4f).renderBackground { ctx, _, _, _ ->
                                    ctx.batchRenderTextureColored {
                                        pushWidgetTexture(transform, WidgetTextures.DROP_DOWN_MENU_BACKGROUND)
                                    }
                                }
                            ) {
                                TextLabel(mutableStateBy { id.getValue().asTranslateText() })
                            }
                            Button(
