...
fun IdentifierComponentEditor(
    ...
    return DataComponentWrapperDialog(
        title,
        { Identifier.of(namespaceState.getValue(), pathState.getValue()) to true },
        onValueChange,
        modifier,
        screenModifier,
        {
            val namespaceValid = Identifier.isNamespaceValid(namespaceState.getValue())
            if (!namespaceValid) {
                namespaceEditor?.let {
            ...
            val pathValid = Identifier.isPathValid(pathState.getValue())
            if (!pathValid) {
                pathEditor?.let {
                    TipHandler.pushTip(IDENTIFIER_COMPONENT_WRAPPER, 5.seconds, it, Tip {
                        TextLabel(
