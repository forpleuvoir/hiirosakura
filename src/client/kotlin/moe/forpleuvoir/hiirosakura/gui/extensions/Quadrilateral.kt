...
fun DrawContext.renderQuad(quad: Quadrilateral, color: ARGBColor, layer: RenderLayer = RenderLayer.getGui()) =
    renderQuad(matrices, vertexConsumers, quad, color, layer)
...
fun renderQuad(
    ...
    bufferBuilder.vertex(matrices, quad.p1).color(color)
    bufferBuilder.vertex(matrices, quad.p2).color(color)
    bufferBuilder.vertex(matrices, quad.p3).color(color)
    bufferBuilder.vertex(matrices, quad.p4).color(color)
    vertexConsumers.draw()
}

fun BoxBatchRenderScope.pushQuad(quad: Quadrilateral, color: ARGBColor) {
    bufferBuilder.vertex(matrices, quad.p1).color(color)
    bufferBuilder.vertex(matrices, quad.p2).color(color)
    bufferBuilder.vertex(matrices, quad.p3).color(color)
    bufferBuilder.vertex(matrices, quad.p4).color(color)
