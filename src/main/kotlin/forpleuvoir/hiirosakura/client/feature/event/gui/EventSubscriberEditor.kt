...
class EventSubscriberEditor(
...
) : DialogConfirm(dialogWidth, dialogHeight, "${HiiroSakuraClient.modId}.screen.event.edit".tText().mText, parent) {
    ...
    private fun initName() {
        nameText = LabelText("${HiiroSakuraClient.modId}.screen.event.name".tText().text, left + 2, top).apply {
    ...
    private fun initEventType() {
        eventTypeText = LabelText(
            "${HiiroSakuraClient.modId}.screen.event.type".tText().mText,
    ...
    private fun initEnabled() {
        enabledText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.event.enabled".tText().mText,
                nameInput.x + nameInput.width + 5 + 120 + 5,
                top
            ).apply {
                align = LabelText.Align.CENTER_LEFT
            }
        enabledButton =
            ButtonOnOff(enabledText.x, enabledText.y + enabledText.height - 2, 60, status = enabledValue).apply {
                setOnOffText(
    ...
    private fun initRunnerType() {
        runnerTypeText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.event.runnerType".tText().mText,
                enabledButton.x + enabledButton.width + 5,
                top
            ).apply {
                align = LabelText.Align.CENTER_LEFT
            }
    ...
    private fun initDelay() {
        delayText = LabelText(
            "${HiiroSakuraClient.modId}.screen.timertask.delay".tText().mText,
            left + 2,
    ...
    private fun initPeriod() {
        periodText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.timertask.period".tText().mText,
        ...
        periodInput =
            WidgetIntInput(periodText.x, periodText.y + periodText.height, 60, 16, periodValue, minValue = 1).apply {
                setOnValueChangedCallback {
                    periodValue = it
                }
            }
    ...
    private fun initTimes() {
        timesText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.timertask.times".tText().mText,
        ...
        timesInput =
            WidgetIntInput(timesText.x, timesText.y + timesText.height, 60, 16, timesValue, minValue = 1).apply {
                setOnValueChangedCallback {
                    timesValue = it
                }
            }
    ...
    private fun initRunAt() {
        runAtText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.timertask.runAt".tText().mText,
    ...
    private fun initFromTimerTask() {
        ...
        fromTimerTaskText =
            LabelText(
                "${HiiroSakuraClient.modId}.screen.event.fromTimerTask".tText().mText,
