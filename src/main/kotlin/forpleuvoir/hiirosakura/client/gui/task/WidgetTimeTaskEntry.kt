...
class WidgetTimeTaskEntry(
    ...
    init {
        ...
        posX -= this.addButton(posX, posY, StringUtil.translatableText("gui.button.run").key) {
            TimeTaskHandler.INSTANCE!!.addTask(entry.timeTask)
            TimeTaskHandler.INSTANCE!!.addTask(
                TimeTask.once(
                    5,
                    "#Close_QTTE_Screen"
                ) {
                    this.parent.parentScreen?.onClose()
                }
            )
        }
